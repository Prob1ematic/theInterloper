...
public class TargetAction{
    ...
    public boolean canTargetPlayer = true;
    ...
    private void updateTargetMode(){
        ...
        if (InputHelper.justClickedLeft || InputHelper.isMouseDown) {
            InputHelper.justClickedLeft = false;
            if (this.hoveredCreature != null) {
                AbstractHeropower power = HeropowerField.heropower.get(AbstractDungeon.player);
                if(power != null){
                    power.use(hoveredCreature);
                }
                close();
            }
        }

        if (InputHelper.justClickedRight || InputHelper.isMouseDown_R) {
